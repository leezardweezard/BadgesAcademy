...
//action for adding meta boxesF
add_action('add_meta_boxes','bsp_add_meta_box1');

//function for adding a meta box
function bsp_add_meta_box1(){
	//slug, name, callback function, post type and priority-where to show the meta box
	add_meta_box('bsp_mother_language1', 'Other Language', 'bsp_show_metabox1', 'badge', 'normal' );
}

//function for showing the metabox on admin menu
function bsp_show_metabox1($post){
	//security check
	wp_nonce_field( basename( __FILE__ ), 'bsp_nonce' );
	//geeting the content of a post where metabox is
    $bsp_stored_meta1 = get_post_meta( $post->ID );
    //html code for actualy showing the metabox
    ?>
    <p style="overflow: auto;"><!--label -->
         <input type="text" name="otherlang">
        <!-- textarea where the content for metabox is placed -->
        <textarea name="bsp_meta-text1" id="bsp_meta-text1" class="widefat" style="overflow:auto; resize:none" rows="15"><?php if ( isset ( $bsp_stored_meta1['bsp_meta-text'] ) ) echo $bsp_stored_meta1['bsp_meta-text'][0]; ?></textarea>
...
//action for adding meta boxesF
add_action('add_meta_boxes','bsp_add_meta_box2');

//function for adding a meta box
function bsp_add_meta_box2(){
	//slug, name, callback function, post type and priority-where to show the meta box
	add_meta_box('bsp_mother_language2', 'Other Language', 'bsp_show_metabox2', 'badge', 'normal' );
}

//function for showing the metabox on admin menu
function bsp_show_metabox2($post){
	//security check
	wp_nonce_field( basename( __FILE__ ), 'bsp_nonce' );
	//geeting the content of a post where metabox is
    $bsp_stored_meta2 = get_post_meta( $post->ID );
    //html code for actualy showing the metabox
    ?>
    <p style="overflow: auto;"><!--label -->
         <input type="text" name="otherlang1">
        <!-- textarea where the content for metabox is placed -->
        <textarea name="bsp_meta-text2" id="bsp_meta-text2" class="widefat" style="overflow:auto; resize:none" rows="15"><?php if ( isset ( $bsp_stored_meta2['bsp_meta-text'] ) ) echo $bsp_stored_meta2['bsp_meta-text'][0]; ?></textarea>
		...
		wp_enqueue_style('bsp_style',plugins_url('css/style.css', __FILE__));
			...
			echo $before_title.$title.$after_title;
			 ...
			 //getting  languages
                       ...
                       $languages= $wpdb->get_results("SELECT short_code,ref_name FROM $table_name",ARRAY_A);
            
                        
                     
//                        $language = $wpdb->get_col("SELECT ref_name FROM $table_name");                                                 
			...
			<?php
				...
				echo '<option value="'.$language["ref_name"].'">'.$language["ref_name"].'</option>';
			  }?>
			</select>
			</div>
			</br>
